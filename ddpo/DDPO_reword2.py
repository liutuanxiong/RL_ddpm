...
def compute_loss(x_T, x, original_log_probs, advantages, clip_advantages, clip_ratio, eta):
    ...
    for i, time_step in enumerate(range(10,-1, -1)):
        ...
        # print(ratio)
        clipped_advantages = clipped_advantages.squeeze(1)
        unclipped_loss = torch.mul(-clipped_advantages , ratio)  # this is the surrogate loss
        # print_cuda("p2 " + str(i) + " 10")
        clipped_loss = torch.mul(clipped_advantages , torch.clip(ratio, 1. - clip_ratio,
...
modelConfig = {
    ...
    "img_size": 32,
    "grad_clip": 1.,
    "clip_advantages": 200000,
    "clip_ratio":0.9999999,
...
for epoch in range(modelConfig["epoch"]):
    ...
    for i, batch in enumerate(train_dataloader):
        with torch.no_grad():
            ...
            img, batch_advantages, batch_all_step_preds, batch_log_probs = sample_and_calculate_rewards(x_T,
                                                                                                        source_images,
                                                                                                        1, origin_t,
                                                                                  target_age,device)
        ...
        if len(all_step_preds) % modelConfig["train_batch"] == 0:
            num +=1
            all_step_preds = torch.cat(all_step_preds, dim=1)
            log_probs = torch.cat(log_probs, dim=1)
            advantages = torch.cat(advantages)
            all_source_images = torch.cat(all_source_images, dim=0)
            all_step_preds = torch.chunk(all_step_preds,
                                         modelConfig["num_samples_per_epoch"] // modelConfig["batch_size"],
                                         dim=1)
            log_probs = torch.chunk(log_probs, modelConfig["num_samples_per_epoch"] // modelConfig["batch_size"],
                                    dim=1)
            advantages = torch.chunk(advantages, modelConfig["num_samples_per_epoch"] // modelConfig["batch_size"],
                                     dim=0)
            ...
            for inner_epoch in range(modelConfig["num_inner_epochs"]):
                ...
                with tqdm(range(len(all_step_preds))) as tqdmloader:
                    for i in tqdmloader:
                        ...
                        # print_cuda("p2 " + str(i) + " 4")
                        optimizer.step()
                        show_epoch = num*modelConfig["num_inner_epochs"]+inner_epoch
                        # print_cuda("p2 " + str(i) + " 5")
                        ...
                        # print_cuda("p2 "+str(i))
                        torch.save(model.state_dict(), "../model/RLmodel_reword2.pth")
                        if show_epoch % 1 == 0:
                            loss_get0 = [np.array(l) for l in loss_get]
                            loss_get0 = np.array(loss_get0)
                            np.save('../loss_data/RL_loss_reword2_get', loss_get0)
            # print_cuda("p0 " + str(i))
            del all_step_preds, all_source_images, log_probs,advantages
            torch.cuda.empty_cache()
            all_step_preds, log_probs, advantages, all_source_images = [], [], [], []
