...
def normalize(array):
    ...
    standardized_array = array - mean_val
    return standardized_array


def RL_scoring(pred,origin,device):
    pred = ones(pred)
    origin = ones(origin)
...
def sample_and_calculate_rewards(x_T, x,eta,origin_t,device):
    preds,all_step_preds, log_probs = sd_sample(x_T, x,eta)
    imgs = ones(preds)
    rewards = RL_scoring(preds,origin_t, device)
...
for epoch in range(modelConfig["epoch"]):
    ...
    for i, batch in enumerate(train_dataloader):
        with torch.no_grad():
            print(i)
            source_images, target_images, origin_s, origin_t,a = batch
            ...
            for inner_epoch in range(modelConfig["num_inner_epochs"]):
                ...
                with tqdm(range(len(all_step_preds))) as tqdmloader:
                    for i in tqdmloader:
                        ...
                        # print_cuda("p2 "+str(i))
                        torch.save(model.state_dict(), "../model/RLmodel_reword1_model.pth")
                        if show_epoch % 1 == 0:
                            loss_get0 = [np.array(l) for l in loss_get]
                            loss_get0 = np.array(loss_get0)
                            np.save('../loss_data/RL_loss_reword1_get', loss_get0)
            # print_cuda("p0 " + str(i))
